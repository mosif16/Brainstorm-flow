...
start_service() {
  ...
  nohup npm --prefix "$path" run dev >"$log_file" 2>&1 &
  local pid=$!
  disown "$pid"
...
BACKEND_PID=$(start_service "backend" "$SERVER_DIR" "$LOG_DIR/backend-dev.log")
FRONTEND_PID=$(start_service "frontend" "$CLIENT_DIR" "$LOG_DIR/frontend-dev.log")
